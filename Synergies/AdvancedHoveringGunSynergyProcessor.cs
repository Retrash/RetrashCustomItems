...
namespace Blunderbeast
{
    public class AdvancedHoveringGunSynergyProcessor : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (this.Trigger == AdvancedHoveringGunSynergyProcessor.TriggerStyle.CONSTANT)
            {
                if (this.m_gun)
                {
                ...
                {
                    if (this.m_item && this.m_item.Owner && this.m_item.Owner.PlayerHasActiveSynergy(this.RequiredSynergy))
                    {
            ...
            else if (this.Trigger == AdvancedHoveringGunSynergyProcessor.TriggerStyle.ON_DAMAGE)
            {
            ...
            else if (this.Trigger == AdvancedHoveringGunSynergyProcessor.TriggerStyle.ON_ACTIVE_ITEM)
            {
                if (!this.m_actionsLinked && this.m_gun && this.m_gun.CurrentOwner)
                {
        ...
        {
            if (sourcePlayer.PlayerHasActiveSynergy(this.RequiredSynergy) && this.GetOwner())
            {
                for (int i = 0; i < this.NumToTrigger; i++)
                {
                    int num = 0;
        ...
        {
            if (sourcePlayer.PlayerHasActiveSynergy(this.RequiredSynergy))
            {
                for (int i = 0; i < this.NumToTrigger; i++)
                {
                    int num = 0;
